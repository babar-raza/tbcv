...
class MarkdownValidatorAgent(BaseValidatorAgent):
    ...
    async def validate(self, content: str, context: Dict[str, Any]) -> ValidationResult:
        ...
        link_issues = self._check_links(lines)
        issues.extend(link_issues)

        # Check for missing alt text in images
        image_issues = self._check_images(lines)
        issues.extend(image_issues)

        # Check for common formatting issues
        format_issues = self._check_formatting(lines)
    ...
    def _check_code_blocks(self, lines: List[str]) -> List[ValidationIssue]:
        """Check for unclosed code blocks."""
        issues = []
        in_code_block = False
    ...
    def _check_links(self, lines: List[str]) -> List[ValidationIssue]:
        """Check for invalid link syntax."""
        issues = []
    ...
    def _check_images(self, lines: List[str]) -> List[ValidationIssue]:
    ...
    def _check_formatting(self, lines: List[str]) -> List[ValidationIssue]:
        """Check for common formatting issues."""
        issues = []
