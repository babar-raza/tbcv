...
def check_ollama():
    ...
    if not ollama.enabled:
        pytest.skip("Ollama is disabled in config (set llm.enabled=true)")
    
    try:
        models = ollama.list_models()
        if not models:
            pytest.skip("Ollama has no models available")
        print(f"\n✓ Ollama available with models: {[m.get('name') for m in models]}")
...
async def setup_full_stack(check_ollama):
    ...
    # Cleanup
    for agent_id in ["orchestrator", "content_enhancer", "llm_validator", 
...
async def test_full_stack_single_file_workflow(setup_full_stack):
    ...
    # Create a test markdown file if needed
    test_md = Path("test_full_stack.md")
    test_content = """---
...
"""
    ...
    if mode == "two_stage":
        ...
        print("  ✓ Two-stage pipeline verified (heuristic + LLM)")
        ...
        print("  ✓ Heuristic-only pipeline verified")
    elif mode == "llm_only":
        assert validation_result.get("llm_validation") is not None, \
            "LLM validation should run in llm_only mode"
        print("  ✓ LLM-only pipeline verified")
    ...
    print(f"  ✓ Issue structure validated")
    ...
    print(f"✓ Enhancement completed in {enhancement_duration:.2f}s")
    ...
    # STAGE 3: Verify database persistence
    print("\nSTAGE 3: Verifying database persistence...")
    
    records = list_validation_records(file_path=test_file, limit=10)
    assert len(records) > 0, "Validation results should be persisted to database"
    
    latest_record = records[0]
    print(f"✓ Found {len(records)} validation record(s) in database")
    ...
    print(f"{'='*80}")
    print(f"✓ Validation: {validation_duration:.2f}s")
    print(f"✓ Enhancement: {enhancement_duration:.2f}s")
    print(f"✓ Total time: {validation_duration + enhancement_duration:.2f}s")
...
async def test_full_stack_directory_workflow(setup_full_stack):
    ...
    # Use test directory from environment or create temp directory
    test_dir = Path(os.getenv("TEST_DIR", "test_full_stack_dir"))
...
"""
    ...
    print(f"✓ Batch validation completed in {batch_duration:.2f}s")
    ...
    print(f"✓ Average time per file: {batch_duration / len(test_files):.2f}s")
...
"""
    ...
    test_file = Path("test_mode_switching.md")
    ...
    for mode in modes_to_test:
        ...
        results[mode] = result
        print(f"  ✓ {mode}: {len(result.get('final_issues', []))} issues, "
    ...
    if settings.llm.enabled:
        ...
        print("✓ LLM-only mode skipped heuristic stage")
