...
def check_pytest_collection(file_path: Path) -> bool:
    ...
    try:
        ...
        if match:
            ...
            print(f"   ✓ Pytest collected {count} tests")
            return count > 0
        else:
            print(f"   ✗ Pytest could not collect tests")
            print(f"\n   Pytest output:")
            print(output)
            return False
...
def main():
    ...
    print("\n" + "=" * 80)
    if all_passed:
        print("✓ DIAGNOSIS PASSED - File should work with pytest!")
        print("\nRun tests with:")
        print(f"  python -m pytest {test_file} -v")
        print("  python all_tests.py")
        return 0
    else:
        print("✗ DIAGNOSIS FAILED - See errors above")
        return 1
